...
const fragmentShaderSource = `
    ...
    float angleBetweenVectors(vec3 vector1, vec3 vector2) {
    ...
    void main() {
        ...
        if (distance < maxDistance) {
            ...
            float thetaAngle = angleBetweenVectors(lightDirection, normal);
            vec3 perfectReflectionR = reflect(-lightDirection, normal);
            float phiAngle = angleBetweenVectors(perfectReflectionR, rayDirection);

            float lightIntensityI = 1.0;
            float alpha = 2.0;

            vec3 diffuseAndPhongShading = lightIntensityI * vec3(0.3, 0.9, 0.3) * (max(0.0, cos(thetaAngle)) + pow(max(0.0, cos(phiAngle)), alpha));
            ...
            if(useCBS){
                ...
                color = (12.0*diffuseAndPhongShading + 1.0*vec3(curvature)) /13.0;
            }else{
                //Basic diffuse shading with a fixed light direction K_d and K_s are the same here
                color = diffuseAndPhongShading;
            }
